// Cloudflare Workers function for Claude API integration
// This function enhances blog post content with AI-generated suggestions

export async function handleEnhance(request, env, ctx) {
    ...
    async function callAI(prompt, maxTokens = 1500) {
      if (model === 'chatgpt') {
        if (!env.OPENAI_API_KEY) {
          throw new Error('OPENAI_API_KEY is not configured. Please add it in Cloudflare Workers dashboard under Settings > Environment variables.');
        }
        return callChatGPT(prompt, maxTokens);
      } else {
        if (!env.CLAUDE_API_KEY) {
          throw new Error('CLAUDE_API_KEY is not configured. Please add it in Cloudflare Workers dashboard under Settings > Environment variables.');
        }
...
Return only the title, nothing else. Make it intriguing and click-worthy while being accurate.`;
    ...
    let mainPrompt = `You're a curious curator who loves discovering interesting patterns and connections. Write in a simple, direct, informal tone - like sharing a fascinating discovery with a friend.
